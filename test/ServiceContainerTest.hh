...
final class ServiceContainerTest {
  ...
  public function container(Assert $assert): void {
    $container =
      new ServiceContainer([new LoggerFactory(), new HttpClientFactory()]);
  ...
  public function lookupByClassNameString(Assert $assert): void {
    $container =
      new ServiceContainer([new LoggerFactory(), new HttpClientFactory()]);
    $assert->mixed($container->lookup((string) Logger::class))
      ->isTypeOf(LoggerFactory::class);
    $assert->mixed($container->lookup((string) HttpClient::class))
